...
import enum
import datetime
from DotParser import DotParser, Node, Edge, Graph, GraphType
import math
...
class QGraphViz_Core(QWidget):
    ...
    def paintGraph(self, graph, painter):
        ...
        for i,edge in enumerate(graph.edges):
            ...
            dist = math.sqrt((gdpos[0] - gspos[0]) * (gdpos[0] - gspos[0]) + (gdpos[1] - gspos[1]) * (gdpos[1] - gspos[1]))
            offsetAmount = 25
            offsetDestx = gdpos[0] - ((offsetAmount * (gdpos[0] - gspos[0])) / dist)
            offsetDesty = gdpos[1] - ((offsetAmount * (gdpos[1] - gspos[1])) / dist)

            offsetSourcex = gspos[0] - ((offsetAmount * (gspos[0] - gdpos[0])) / dist)
            offsetSourcey = gspos[1] - ((offsetAmount * (gspos[1] - gdpos[1])) / dist)
            ...
            arrowPoint1x = int(offsetDestx-unitDx * arrowHeadBoxSize - unitDy * arrowHeadBoxSize)
            arrowPoint1y = int(offsetDesty-unitDy * arrowHeadBoxSize + unitDx * arrowHeadBoxSize)

            arrowPoint2x = int(offsetDestx - unitDx * arrowHeadBoxSize + unitDy * arrowHeadBoxSize)
            arrowPoint2y = int(offsetDesty - unitDy * arrowHeadBoxSize - unitDx * arrowHeadBoxSize)

            painter.drawLine(offsetDestx, offsetDesty, arrowPoint1x, arrowPoint1y)
            painter.drawLine(offsetDestx, offsetDesty, arrowPoint2x, arrowPoint2y)
